...
# wt_counts_se <- airway[grep("^ENSG", rownames(airway)),] 
# wt_counts_se <- wt_counts_se[rowMeans(assay(wt_counts_se)) > 10,] # filter row counts less than 10
# head(assay(wt_counts_se))

wt_counts <- readr::read_tsv("./data/wt_counts")
names(wt_counts)[1] <- "ENSEMBL"
wt_counts <- wt_counts %>% dplyr::select(ENSEMBL, wt_TPOS1:wt_VEHS4)
rownames(wt_counts) <- wt_counts$ENSEMBL
countdata <- wt_counts %>% dplyr::select(-ENSEMBL)

# The data is read as a `SummarizedExperiment` object.
